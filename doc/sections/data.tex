	...
	\item for each officer profile, we attempt to \emph{match} it against the
		...
		\begin{itemize}
			...
			\item if the match is unsuccessful, we add a new officer (to which
				...
				The newly added officer will thus be associated with a single
...
Thus, at the end of this procedure, the \texttt{final/} folder contains:  (1)
...
step 2.\ of the procedure described in the previous paragraph.


Our matching
procedure to link officers across datasets thus needs to strike a balance
between:
\begin{itemize}
